...
# Vendored from mlx-whisper 0.4.3 (MIT) — see this package's __init__.py.
...
def transcribe(
    ...
    **decode_options,
    ...
    # VENDORED CHANGE: word-level timestamps (timing.py) were removed with the
    # numba + scipy dependency tree; dictation only needs the text.
    if word_timestamps:
        raise ValueError(
            "word_timestamps is not supported in chad's vendored whisper "
            "(the numba/scipy timing path was removed)"
        )
