"""ATIF (Agent Trajectory Interchange Format) v1.7 trajectory emitter — plan 068.
...
**Segments.** A trial can span several `Agent` objects: plan 040 relaunches a fresh turn
...
class TrajectoryRecorder:
    ...
    def __init__(self, path: str, agent_name: str = "chad",
                 agent_version: str = "0.1.0", extra: Optional[dict] = None):
        self.path = path
...
def recorder() -> Optional[TrajectoryRecorder]:
    ...
    process so plan-040 relaunches (each a fresh Agent) land in one trajectory document."""
