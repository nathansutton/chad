...
from .audio import PreprocessArgs, get_logmel
...
class BaseParakeet(nn.Module):
    ...
    def transcribe(
        ...
        *,
        ...
        """
        # VENDORED CHANGE: file transcription went with load_audio (ffmpeg) —
        # chad's dictation path is get_logmel(mic array) + generate().
        raise NotImplementedError(
            "vendored copy: transcribe(path) was removed with the ffmpeg loader; "
            "use generate(get_logmel(audio, preprocessor_config))"
        )
