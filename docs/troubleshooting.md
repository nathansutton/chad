...
| First token slow after `/compact` or a truncation | one-time full re-prefill (the cache is non-trimmable); the status line shows an advancing % | expected — this is the cache trade chad lives with; see [Design & internals](design.md) |
| Mac swap-storming with other apps open | ~12 GB of weights plus a ~4.3 GB prefill transient plus your apps exceed free RAM | close apps; or lower `CHAD_CTX_SAFETY` to buy headroom back from the context window (see [Memory safety](configuration.md#context-window-agentic-coding-needs-room)) |
