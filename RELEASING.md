...
5. **Version bump** in `pyproject.toml` (dist name is `chad-code`; the command
   stays `chad`). `--version` output follows automatically.
6. **Tag and push**:
   ```bash
...
8. **Benchmark claims stay honest.** If the verified Terminal-Bench number
   ...
   dedicated commit per the pre-committed landing rules (maintainer note:
   private plan 071) — never mixed into a release commit.
