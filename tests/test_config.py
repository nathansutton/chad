...
`config` is where every flag/int/float knob now funnels (plan 055), so its parse rules ARE
...
def test_env_int(monkeypatch):
    ...
    # LENIENT contract (plan 052): a non-numeric value degrades to the default, does NOT
