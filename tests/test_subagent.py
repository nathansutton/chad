"""Tests for the subagent/Task tool + engine cache quarantine.
...
def test_agent_env_parse_tolerates_garbage(monkeypatch):
    ...
    Agent.__init__. Before the fix, int('x')/float('x') raised inside __init__
...
def test_subagent_cannot_write_todos():
    ...
    clobber the parent's pinned todo panel. A sub-agent's plan is not the
